...
{{- $unimplementedCustomSubject := .GetName}}
type Unimplemented{{$unimplementedCustomSubject}}Server struct {}

{{- range .Method}}
{{- if ne .GetInputType ".nrpc.NoRequest"}}
{{- $resultType := GetResultType .}}
func (*Unimplemented{{$unimplementedCustomSubject}}Server) {{.GetName}}(ctx context.Context
	{{- range GetMethodSubjectParams . -}}
	, {{ . }} string
	{{- end -}}
	{{- if ne .GetInputType ".nrpc.Void" -}}
	...
	{{- if ne $resultType ".nrpc.NoReply" }}
	{{- if and (ne $resultType ".nrpc.Void") (not (HasStreamedReply .)) }} (*{{GoType $resultType}}, 
	{{- end }} error
	...
	{{- end }} {
	{{- if ne $resultType ".nrpc.NoReply" }}
	return {{ if and (ne $resultType ".nrpc.Void") (not (HasStreamedReply .)) -}}
    nil, 
    {{- end -}}
	errors.New("method {{.Name}} not implemented")
	{{- end }} 
}
