...
//go:generate protoc -I. -I./third_party -I./third_party/gogoproto --gogo_out=. nrpc_test.proto
...
func TestBasic(t *testing.T) {
	...
	subn, err := nc.Subscribe("foo.*", func(m *nats.Msg) {
		if err := Publish(&DummyMessage{Foobar: "world"}, nil, nc, m.Reply, "protobuf"); err != nil {
			t.Fatal(err)
		}
	})
	if err != nil {
...
func TestDecode(t *testing.T) {
	...
	subn, err := nc.Subscribe("foo.*", func(m *nats.Msg) {
		...
		} else if err := Publish(&DummyMessage{Foobar: "world"}, nil, nc, m.Reply, "protobuf"); err != nil {
			t.Fatal(err)
		}
	})
	if err != nil {
...
func TestStreamCall(t *testing.T) {
	...
	t.Run("Simple stream", func(t *testing.T) {
		subn, err := nc.Subscribe("foo.*", func(m *nats.Msg) {
			...
			if err := Publish(
				nil,
				&Error{Type: Error_EOS, MsgCount: 2},
				nc, m.Reply, "protobuf",
			); err != nil {
				t.Fatal(err)
			}
	...
	t.Run("Error", func(t *testing.T) {
		subn, err := nc.Subscribe("foo.*", func(m *nats.Msg) {
			if err := Publish(
				nil,
				&Error{Type: Error_CLIENT, Message: "error"},
				nc, m.Reply, "protobuf",
			); err != nil {
				t.Fatal(err)
			}
...
func TestError(t *testing.T) {
	...
	subn, err := nc.Subscribe("foo.*", func(m *nats.Msg) {
		if err := Publish(
			&DummyMessage{Foobar: "world"},
			&Error{Type: Error_CLIENT, Message: "anerror"},
			nc, m.Reply, "protobuf",
		); err != nil {
			t.Fatal(err)
		}
	...
	err = Call(&DummyMessage{Foobar: "hello"}, &DummyMessage{}, nc, "foo.bar", "protobuf", 5*time.Second)
	if err == nil {
...
func TestTimeout(t *testing.T) {
	...
	err = Call(&DummyMessage{Foobar: "hello"}, &DummyMessage{}, nc, "foo.bar", "protobuf", 500*time.Millisecond)
	if err == nil {
		t.Fatal("error expected")
...
func TestMarshalUnmarshalResponse(t *testing.T) {
	for _, tt := range encodingTests {
		t.Run("UnmarshalResponse"+tt.encoding, func(t *testing.T) {
			var msg DummyMessage
			err := UnmarshalResponse(tt.encoding, tt.data, &msg)
			if err != nil {
				t.Fatal(err)
			}
			if msg.Foobar != encodingTestMsg.Foobar {
		...
		t.Run("MarshalErrorResponse"+tt.encoding, func(t *testing.T) {
			data, err := MarshalErrorResponse(tt.encoding, &Error{
				Type: Error_SERVER, Message: "Some error",
			})
			if err != nil {
			...
			var msg DummyMessage
			err = UnmarshalResponse(tt.encoding, data, &msg)
			if err == nil {
			...
			if repErr.Type != Error_SERVER || repErr.Message != "Some error" {
...
func TestParseSubject(t *testing.T) {
	for i, tt := range []struct {
		...
		pkgParams, svcParams, name, tail, err := ParseSubject(
		...
		var encoding string
		if err == nil {
			mtParams, encoding, err = ParseSubjectTail(tt.mtParamsCount, tail)
		}
...
func TestCaptureErrors(t *testing.T) {
	t.Run("NoError", func(t *testing.T) {
		msg, err := CaptureErrors(func() (proto.Message, error) {
	...
	t.Run("DirectError", func(t *testing.T) {
		msg, err := CaptureErrors(func() (proto.Message, error) {
			return nil, &Error{Type: Error_SERVER, Message: "anerror"}
		})
		if err == nil {
			t.Fatal("Expected an error, got nothing")
		}
